                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        for hit in hits:
                            ...
                            hiring_match = re.split(
                                r"\s+(?:is hiring|Is Hiring|hiring|—|\:)\s*",
                                raw_title,
                                maxsplit=1,
                            )
                            ...
                            if location == "Unknown":
                                if "onsite in " in job_text.lower():
                                    onsite_match = re.search(
                                        r"onsite in\s+([a-zA-Z\s,]+)",
                                        job_text,
                                        re.IGNORECASE,
                                    )
                                    if onsite_match:
                        ...
                        errors.append(
                            {
                                "source": self.provider_name,
                                "message": f"HN jobs API HTTP error {response.status_code}",
                            }
                        )
